  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-10 md:py-16">
      <div className="page-spotlight -mx-4 mb-10 border-b border-ink/10 px-4 pb-10 md:pb-14">
        <p className="motion-rise text-[10px] font-bold tracking-[0.2em] text-cherry mb-3">YOUR TROPITWIST BAG</p>
      ...
      {catalogStatus === 'loading' && itemCount > 0 ? (
        ...
        <div className="card-enter rounded-brand bg-banana p-10 md:p-16 text-center">
          ...
          <a href={route('/search')} className="inline-block mt-7 rounded-full bg-ink text-cream px-7 py-4 text-xs font-bold tracking-widest hover:bg-cherry transition-colors">VIEW ALL PRODUCTS</a>
        </div>
      ) : (
        <div className="grid lg:grid-cols-[1fr_380px] gap-10 items-start">
          <section className="divide-y divide-ink/10 border-y border-ink/10">
            {items.map(({ product, quantity }, index) => (
              <article key={product.id} className="card-enter py-6 flex gap-4 md:gap-6" style={{ animationDelay: `${Math.min(index * 70, 420)}ms` }}>
                <a href={route(`/products/${product.id}`)} className="w-28 h-32 md:w-36 md:h-40 shrink-0 rounded-brand bg-[#FFF1D8] overflow-hidden">
                ...
                <div className="flex-1 flex flex-col justify-between gap-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex min-h-10 items-center border border-ink/15 rounded-full">
                      <button aria-label={`Decrease ${product.name}`} onClick={() => updateQuantity(product.id, quantity - 1)} className="px-3 py-2">−</button>
                      <span className="w-7 text-center text-xs">{quantity}</span>
                      <button aria-label={`Increase ${product.name}`} onClick={() => updateQuantity(product.id, quantity + 1)} className="px-3 py-2">+</button>
                    </div>
          ...
          <aside className="card-enter rounded-brand bg-[#FFF1D8] p-6 md:p-8 lg:sticky lg:top-28">
