    ...
    <main className="max-w-7xl mx-auto px-4 py-10 md:py-16">
      ...
      {items.length === 0 ? (
        <div className="rounded-brand bg-banana p-10 md:p-16 text-center">
          ...
          <a href={route('/skincare')} className="inline-block mt-7 rounded-full bg-ink text-cream px-7 py-4 text-xs font-bold tracking-widest hover:bg-cherry transition-colors">SHOP SKINCARE</a>
        </div>
      ) : (
        <div className="grid lg:grid-cols-[1fr_380px] gap-10 items-start">
          ...
          <aside className="rounded-brand bg-[#FFF1D8] p-6 md:p-8 lg:sticky lg:top-28">
            ...
            <a href={route('/skincare')} className="block text-center mt-5 text-[10px] font-bold tracking-widest hover:text-cherry">CONTINUE SHOPPING</a>
          </aside>
        </div>
      )}
    </main>
