      ...
      <section className="max-w-7xl mx-auto px-4 pt-5 md:pt-8">
        <div className="grid md:grid-cols-[.9fr_1.1fr] min-h-[600px] overflow-hidden rounded-brand">
          <div className="bg-cherry text-cream p-8 md:p-14 flex flex-col justify-between">
            <p className="motion-rise text-[10px] font-bold tracking-[0.22em]">THE TROPITWIST STORY</p>
            <div>
              <h1 className="motion-reveal font-display font-bold text-7xl md:text-[9rem] leading-[0.78] tracking-tight">GLOW<br />IS A<br /><span className="text-banana">MOOD.</span></h1>
              <p className="motion-rise motion-delay-2 mt-8 max-w-sm text-sm text-cream/80 leading-relaxed">Tropitwist is skincare for the bright side of real life: uncomplicated, expressive, and made to move with you.</p>
            </div>
          ...
          <div className="bg-banana stripe-yellow-tight min-h-[440px] relative overflow-hidden">
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="motion-orbit w-64 h-64 md:w-96 md:h-96 rounded-full border-[24px] border-cherry/90 flex items-center justify-center">
            ...
            <span className="absolute bottom-8 right-8 text-right font-display text-3xl text-cherry md:text-4xl">SUN ON.<br />GLOW ON.</span>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-20 md:py-28">
        <div className="grid md:grid-cols-[.7fr_1.3fr] gap-10 md:gap-20">
          ...
          <div>
            ...
            <div className="motion-rise motion-delay-2 flex flex-wrap gap-3 mt-8"><a href={route('/skincare')} className="rounded-full bg-ink text-cream px-6 py-3 text-xs font-bold tracking-widest hover:bg-cherry transition-colors">SHOP SKINCARE</a><a href={route('/bundles')} className="rounded-full border border-ink/20 px-6 py-3 text-xs font-bold tracking-widest hover:border-cherry hover:text-cherry transition-colors">EXPLORE BUNDLES</a></div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-ink text-cream">
        <div className="max-w-7xl mx-auto px-4 py-16 md:py-24">
          ...
          <div className="grid md:grid-cols-3 border-t border-cream/15">
            {VALUES.map((value, index) => <article key={value.number} className="card-enter pt-6 md:pr-10 md:border-r md:border-cream/15 md:mr-10 mt-8 last:border-0" style={{ animationDelay: `${index * 90}ms` }}>
