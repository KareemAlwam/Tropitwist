...
export default function Admin() {
  ...
  async function updateProduct(id, changes) {
    try {
      ...
      setDashboard((current) => ({ ...current, products: current.products.map((item) => item.id === id ? { ...item, ...product, price: `LE ${product.price}` } : item) }));
...
function Products({ products, onUpdateProduct, onCreateProduct }) {
  ...
  async function submit(event) {
    ...
    const created = await onCreateProduct({ ...values, price: Number(values.price), inventory: Number(values.inventory), featured: values.featured === 'on', bestseller: values.bestseller === 'on', status: values.status || 'draft', detail: values.detail || '' });
    if (created) setAdding(false);
  }
  return <section className="rounded-brand bg-cream p-6 shadow-sm md:p-8"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold tracking-[0.2em] text-cherry">CATALOG</p><h2 className="mt-2 font-display text-2xl font-bold">Product catalog</h2></div><button type="button" onClick={() => setAdding((value) => !value)} className="rounded-full bg-cherry px-5 py-3 text-[10px] font-bold tracking-widest text-cream transition-colors hover:bg-ink">{adding ? 'CANCEL' : 'ADD PRODUCT'}</button></div>{adding && <form onSubmit={submit} className="mt-6 grid gap-3 border-y border-ink/10 py-5 sm:grid-cols-2"><input name="name" placeholder="PRODUCT NAME" required className="checkout-input" /><input name="slug" placeholder="product-slug" required className="checkout-input" /><select name="category" className="checkout-input"><option value="skincare">Skincare</option><option value="body">Body care</option><option value="bundles">Bundles</option></select><input name="type" placeholder="TYPE" required className="checkout-input" /><input name="price" type="number" min="0" placeholder="PRICE" required className="checkout-input" /><input name="inventory" type="number" min="0" placeholder="INVENTORY" required className="checkout-input" /><input name="size" placeholder="SIZE" className="checkout-input" /><select name="status" className="checkout-input"><option value="draft">Draft</option><option value="active">Active</option></select><label className="flex min-h-11 items-center gap-2 text-xs font-bold tracking-wide"><input name="featured" type="checkbox" /> SHOW IN BESTSELLERS</label><label className="flex min-h-11 items-center gap-2 text-xs font-bold tracking-wide"><input name="bestseller" type="checkbox" /> BESTSELLER BADGE</label><input name="image" type="url" placeholder="IMAGE URL" required className="checkout-input sm:col-span-2" /><textarea name="description" placeholder="DESCRIPTION" required className="checkout-input sm:col-span-2" /><textarea name="detail" placeholder="DETAILS (OPTIONAL)" className="checkout-input sm:col-span-2" /><button className="w-fit rounded-full bg-ink px-5 py-3 text-[10px] font-bold tracking-widest text-cream hover:bg-cherry">CREATE PRODUCT</button></form>}{!products.length ? <StateMessage>No products have been returned by the catalog service.</StateMessage> : <div className="mt-6 overflow-hidden rounded-brand border border-ink/10">{products.map((product) => <div key={product.id} className="grid gap-3 border-b border-ink/10 p-4 last:border-0 sm:grid-cols-[1fr_auto_auto_auto_auto_auto] sm:items-center"><div><p className="font-bold">{product.name}</p><p className="text-xs capitalize text-ink/55">{product.category} · {product.size}</p></div><input type="number" min="0" defaultValue={product.inventory ?? 0} aria-label={`Inventory for ${product.name}`} onBlur={(event) => onUpdateProduct(product.id, { inventory: Number(event.target.value) })} className="w-20 rounded border border-ink/15 px-2 py-1 text-xs" /><label className="flex items-center gap-2 text-[10px] font-bold tracking-wide"><input type="checkbox" checked={Boolean(product.featured)} aria-label={`Show ${product.name} in Bestsellers`} onChange={(event) => onUpdateProduct(product.id, { featured: event.target.checked })} /> HOME</label><label className="flex items-center gap-2 text-[10px] font-bold tracking-wide"><input type="checkbox" checked={Boolean(product.bestseller)} aria-label={`Show bestseller badge for ${product.name}`} onChange={(event) => onUpdateProduct(product.id, { bestseller: event.target.checked })} /> BADGE</label><select value={product.status || 'active'} aria-label={`Status for ${product.name}`} onChange={(event) => onUpdateProduct(product.id, { status: event.target.value })} className="rounded-full bg-[#E8F3EC] px-3 py-1 text-xs font-bold text-[#2F6B4F]"><option value="active">active</option><option value="draft">draft</option></select><span className="font-bold">{product.price}</span></div>)}</div>}</section>;
}
